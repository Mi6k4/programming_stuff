...
import os
import pandas as pd
from mitosheet import TYPE, IF, PROPER, AND, CONCAT, to_int_series, FIND, SUBSTITUTE, LEFT, INT, CLEAN, to_float_series, FLOAT, FILLNAN, LEN, RIGHT, OR, DATEVALUE, TEXT    # noqa: E501
import yadisk
import psycopg2
...
def zemchuzina_volgograd(file_name, file_path):
    wb = load_workbook(file_name, data_only=True)
    sheet_name = wb.sheetnames[0]
    sheet_df_dictonary = pd.read_excel(file_name, engine='openpyxl', sheet_name=[sheet_name], skiprows=0, header=None)
    Sheet_Name_Here = sheet_df_dictonary[sheet_name]
    # Renamed columns
    Sheet_Name_Here.rename(columns={0: '№ полиса',
                                    1: 'ФИО',
                                    2: 'Код услуги',
                                    3: 'Наименование услуги',
                                    4: '№ зуба',
                                    ...
                                    7: 'Кол-во',
                                    9: 'Диагноз',
                                    10: 'Код по МКБ'}, inplace=True)
    # Filtered
    Sheet_Name_Here = Sheet_Name_Here[(Sheet_Name_Here['Код услуги'].notnull()) & (Sheet_Name_Here['Код услуги'] != 'Код')]
    # Changed dtype
    Sheet_Name_Here['Кол-во'] = to_int_series(Sheet_Name_Here['Кол-во'])
    Sheet_Name_Here['Цена'] = to_float_series(Sheet_Name_Here['Цена'])
    df = pd.DataFrame.from_dict(Sheet_Name_Here)
...
def template_qualifier(folder_name, printer=False):
    template_tuple = {
        ...
        '"Дальвен", ООО': dalven,
        '«Стоматология на Таганской», ООО': data_pandas_stomat_tagansk,
        '«Клиники Чайка», ООО': chaika,
        '"Стоматологическая поликлиника №9 Азино", ООО': data_pandas_azino,
        'Клиника "Креде эксперто", ООО': data_pandas_kredo_experto,
        'ЧУЗ "ЦКБ "РЖД-медицина"': data_pandas_rzd,  # data_pandas_rzd_fix (Старый формат)
        '«МЦ» XXI ВЕК», ООО': vek_21,
        '"СМАРТ КЛИНИК", ООО': smart_clinic,
        '«ГУТА-КЛИНИК», ООО': guta_clinic,
        'ГБУЗ «ГКБ им. И.В. Давыдовского ДЗМ»': davidovskogo,
        '«Поликлиника «Медросконтракт»': medroscontract,
        '«ЛиМ», ООО': lim,
        '«Клиника современных технологий «Садко», ООО': sadoko_kst,
        '«Центр развития стоматологии «Садко», ООО': sadoko_crs,
        'ГБУЗ "ГКБ им. С.С. Юдина ДЗМ"': udina,
        '«СФЕРА-МЕД», ООО': sphera_med,
        'ГБУЗ "ГКБ им. А.К.Ерамишанцева ДЗМ"': gbuz_eramishanceva,
        '"МедГард" (Самара, Тольятти), ООО': medgard_samara_toliati,
        '"МедГард" (Невский), ООО': medgard_nevskii,
        '"Медицина Альфастрахования"(Моска), ООО': alfasrahovanie_moscow,
        '«Медицина АльфаСтрахования г. Ростов-на-Дону», ООО': alfasrahovanie_rostov_na_donu,
        'КГБУЗ КДЦ "Вивея"': vivea,
        'АО "Детская поликлиника Литфонда"': litfond,
        '"Жемчужина Подолья", ООО': zhemchuzhina_podolia,
        '«Стоматология Стиль Дент», ООО': stil_dent,
        '«Клиника «Аллергомед», ООО': allergomed,
        ...
        'УГМК': ugmk,
        '«Эстетика», ООО': estetika,  # Ручное (частично)
        '«Клиника ОсНова»': osnova,
        'ФГБУ «Клиническая больница» Управления делами Президента Российской Федерации': look_data,
        'Центр современной медицины, ООО': center_sovremennoy_medecini,
        'Сеть семейных медицинских центров, ООО': set_semeynih_med_centrov,
        'ФГБУ "НМИЦ ТПМ" МИНЗДРАВА РОССИИ': nmictpm_mz_rf,
        'АО "Семейный доктор"': look_data,
        '32 практика.xls': praktica_32,
        'Ависмед.xls': avismed,
        '"МЦ Айболит", ООО': aibolit,
        '"Аллегро", ООО': allegro,
        'ГБУЗ "ГКБ имени В.М Буянова ДЗМ"': buyanova,
        '«МЦ «В Надежных Руках», ООО': v_nadezhnih_rukah,
        '«ФИРМА ВЕРОНИКА ЛТД», ООО': veronika,
        'Витоника': vitonika,
        'Ворохобова': vorohobova,
        'Гк Медицина': gk_medecina,
        'ФГУП "ГлавУпДК" при МИД России': look_data,
        '«ИНТАН» 1-Й ЦЕНТР ЗУБНОЙ ИМПЛАНТАЦИИ И СТОМАТОЛОГИИ», ООО': intan,
        'АО "Ильинская больница"': ilinskaya_bolnica,
        '"Здоровое поколение", ООО': zdorovor_pokolenie,
        ' «Звезда», ООО': zvezda,
        'ЗАО МЕДИ': zao_medi,
        '"Гранти-мед", ООО': granti_med,
        'Ермишанцева': ermishanceva,
        '«ДЖИ ЭМ ЭС», ООО': jms,
        'Центр Стоматологии "Дентиформ", ООО': dentiform,
        '"Дентика", ООО': dentika,
        '"Камелия-Мед", ООО': kamelia_med,
        '"Стоматологическая поликлиника "Лазурь", ООО': lazur,
        ...
        '"Немецкая клиника", ООО': nemeckaya_clinika,
        'Наука, ООО': nauka,
        'МРТ24, ООО': data_pandas_24mrt,
        'Медтим, ООО': medtim,
        '«МЕДИЦИНСКИЙ ЦЕНТР АТЛАС», ООО': medicinskiy_cent_atlas,
        '«Профессиональная медицинская лига», ООО': professionalnaya_medicinskaya_liga,
        'ФГБУ "НМИЦ ТО им Н.Н. Приорова" Минздрава России': pirogova,
        'АО «Поликлинический Комплекс»': policlinicheskiy_komplex,
        '«Семейный Медицинский Центр», ООО': semeiniy_med_centr,
        '«Сити-Клиник», ООО': city_clinik,
        'СК "ТЕСТ", ООО': sk_test,
        'СК "Юнит", ООО': sk_unit,
        'Общество с ограниченной ответственностью "СОН-МЕД"': son_med,
        'Стоматологическая поликлиника 9 Казань': stomat_poli_9_kazan,
        '«СуперМедик», ООО': super_medic,
        'ФГБУ ФНКЦ ФМБА России': fnkc_fmba,
        '"Формула улыбки", ООО': formula_ulibki,
        '«Форум Интернэшнл Технолоджи», ООО': forum_internationla_technolog,
        '«Центр Семейной Медицины», ООО': centr_semeynoy_med,
        'ФГБУ «ЦКБ с поликлиникой»': fgbu_ckb_s_poliklinikoy,
        'ЦНИИСиЧЛХ': look_data,
        '"ЦМРТ Новослободская", ООО': crmt_Novoslobotskaya,
        '«ЦМРТ Сокольники», ООО': crmt_Novoslobotskaya,
        'ГБУЗ НИКИО им.Л.И.Свержевского ДЗМ': gbuz_nikio_im_l_i_sverzhevckogo,
        'ОАО «Фармация»': farmaciya,
        'ОАО «Фармация» (Эскулап)': 'Ручное',
        '"АПЕКС", ООО': apex,
        '"Бека-Инвест", ООО': beka_invest,
        '"ДЕНС", ООО': dems,
        '"КДС", ООО': kds,
        '"Клиника Доброго Стоматолога", ООО': klinika_dobrogo_stomatologa,
        '"МедГард", ООО': medgard,
        '"Медицинские Центры - 2", ООО': medecinskie_centri_2,
        '"Многопрофильный медицинский центр "Диалайн", ООО': mnogoprofilnii_medecinskii_center_dialain,
        '"Одинмед", ООО': odinmed,
        '"Поликлиника № 2 Вита Медикус", ООО': poliklinika_2_vita_medicus,
        '"РМА", ООО': rma,
        '"СОГАЗ-Медсервис", ООО': sogaz_medservis,
        '"Спорткомплекс "Олимпия-Пермь", ООО': stomatkomplex_olimpia_perm,
        '"Стоматбизнес Компани", ООО': stomatbiznes_company,
        '"Медики", ООО': stomatbiznes_company,
        '"Стомед", ООО': stomed,
        '"Династия", ООО': dinastia,
        '"ЭФА", ООО': efa,
        '«Боткинская 33», ООО': botkinskaya_33,
        '«ВАЛЕ-ДЕНТАЛЬ», ООО': vale_dental,
        '«Гутен Таг», ООО': guten_tag,
        '«Доктор рядом», ООО': doctor_ryadom,
        '«Европейский медицинский центр «УГМК-Здоровье», ООО': evropeiskiy_medecinskiy_center_egmk_zdorovie,
        '«Клиника немецкой стоматологии «Гутен Таг», ООО': klinika_nemeckoy_stomatologii_guten_tag,
        '«Клиника санитас в медпарке», ООО': klinika_sanitas_v_medparke,
        '«Медицина АльфаСтрахования г. Пермь», ООО': medecina_alfastrahovaniya_perm,
        '«Медицина АльфаСтрахования г. Самара», ООО': medecina_alfastrahovaniya_samara,
        '«Медицина АльфаСтрахования г. Ярославль», ООО': medecina_alfastrahovaniya_yaroslavl,
        '«Мир улыбок», ООО': mir_ulibok,
        '«Наш МЦ «Парацельс», ООО': nash_mc_paracels,
        '«Панацея», ООО': panaceya,
        '«Первый Доктор», ООО': perviy_doctor,
        '«Призвание», ООО': prizvanie,
        '«Приоритет», ООО': prioritet,
        '«Стомат. студия «Ваш Доктор», ООО': stomat_studia_vash_doctor,
        '«Стоматологическая поликлиника «Денто-Смайл», ООО': stomatologicheskay_poliklinika_dento_smail,
        '«Стоматологическая поликлиника ВИЗАВИ», ООО': stomatologicheskay_poliklinika_vizavi,
        '«Стоматоша», ООО': stomatoshka,
        '«Стоматологическая поликлиника №9 города Казани», ООО': stomatologicheskay_poliklinika_9_goroda_kazani,
        '«Стоматологический центр», ООО': stomatologicheskiy_center,
        'АО "Самарский диагностический центр"': samarskiy_dia_centr,
        'ГКБ №1 им. "Н.И. Пирогова"': gkb_nomer_1_n_i_pirogova,
        '"Витаника", ООО': vitanika,
        '"Даймонд клиник", ООО': diamond,
        '"Поликлиника консультативно-диагностическая им. Е.М. Нигинского", ООО': poliklinaka_konsultativno_diagnosticheskaya_im_e_m_nigibskogo,
        '"ЦСВМП", ООО': csvmp,
        '"поликлиника ЛАУС ДЕО", ООО': laus_deo,
        '«Блеск» (на Геодезической), ООО': blesk_na_geodezicheskoi,  # Нужен координатный морфер
        '«СЦ Приор-М», ООО': cs_prior_m,
        '«Тонус+», ООО': tonus_plus,
        '«Тонус», ООО': tonus,
        '«ЦС «32 Практика», ООО': cs_32_praktika,
        '«Центр лучевой диагностики «ТОНУС ПРЕМИУМ», ООО': centr_luchevoi_diagnostiki_tonus_premium,
        '«ЭРАМед», ООО': eramed,
        '"Эра-1", ООО': era_1,
        'ЛДЦ "Казанская клиника", ООО': ldc_kazanskaya_klinka,
        'МЦ «Лотос», ООО': lotos,
        ...
        '"Парацельс" (г. Краснодар), ООО': paracels_krasnodar,
        '«Наш доктор» (п. Мехзавод), ООО': nash_doctor_p_mehzavod,
        'Альфа Мед Плюс, ООО': alfa_med,
        'Альфа Мед, ООО': alfa_med,
        'МаксиДент, ООО': alfa_med,
        'МаксиДент+, ООО': alfa_med,
        'ГБУЗ СО ССП № 3': gbuz_so_ssp_3,
        'Дент-Реал, ООО': dent_real,
        'Дент-Реал Плюс, ООО': dent_real_plus,
        '«МЕДИЦИНА БУДУЩЕГО», ООО': medicina_buduschego,
        ...
        ' МФЦ «Гармония», ООО': mfc_garmoniya,
        '«Медицина АльфаСтрахования г. Мурманск», ООО': medecina_alfastrahovaniya_murmansk,
        '«Медицина АльфаСтрахования г. Тюмень», ООО': medecina_alfastrahovaniya_tumen,
        ...
        '"Эмаль", ООО': emal,
        '"Юни Медика", ООО': uni_medica,
        'Надежда, ООО': nadezhda,
        '«ВитаСмайл», ООО': vita_smile,
        'НМИЦ МЕДИКА МЕНТЕ, ООО': nmic_medica_mente,
        'ВДЦ - медицинский центр, ООО': vdc_med_center,
        'МЦ «Наедине-Н», ООО': mc_naedine_n,
        'АЛДЕНТ, ООО': aldent,
        'ССК ЗУБАСТИК, ООО': ssk_zubastik,
        'ОММЦ им. св.Луки, ООО': ommc_im_cv_luki,
        'Норма ХХI, ООО': norma_xxi,
        'Ной, ООО': noy,
        'Стоматолог и Я, ООО': stomatolog_i_ya,
        'Медосмотр, ООО': medosmotr,
        'ИП Коротчик Ю.О., ООО': ip_korotchik_u_o,
        'Улыбка Плюс, ООО': ulibka_plus,
        'А-Стом, ООО': a_stom,
        'КЛИНИКА СЕМЕЙНОЙ МЕДИЦИНЫ, ООО': klinika_semeynoy_medecini,
        'БУ «Республиканский клинический госпиталь для ветеранов войн» Минздрава Чувашии': bu_rlg_dvv_min_chuvash,
        'НПО Волгоградский центр профилактики болезней ЮгМед, ООО': npo_vcpb_ugmed,
        'ЮгМед-М, ООО': ugmed_minus_m,
        'ЧУЗ «КБ «РЖД-Медицина» им. Н.А. Семашко»': rzd_semashko,
        '"Авиценна", ООО': avicenna,
        'МЦ «ЦУП МедПрофи», ООО': mc_cup_medprofi,
        'Клиника «Персона», ООО': clinika_persona,
        '«Ивстройтех», ООО': ivstroiteh,
        '«Здоровые люди», ООО': zdorovie_ludi,
        '«Совермед», ООО': sovermed,
        '"ЛДЦ МИБС-Чебоксары", ООО': ldc_mibs_cheboksari,
        '«ММЦ «УРО-ПРО», ООО': mmc_urp_pro,
        'Гиппократ, ООО': gippokrat,
        '«РУЗА ДЕНТ», ООО': razu_dent,
        'ЛПЦ "ДентЛиния", ООО': lpc_dent_liniya,
        '«Стоматолог-2», ООО': stomatologiya_2,
        '"КЕЛЛЕР СТАЧКИ", ООО': keller_stachki,
        'ФБУЗ «Медико-санитарная часть № 9» ФМБА России г. Дубна': fbuz_msch_9_fbma_r_dubna,
        '«МЦ на Кырли», ООО': mc_kirli,
        '"Стоматологический Центр "ВладМиВа", ООО': stomat_center_vladmiva,
        '«Национальный Диагностический Центр», ООО': nac_dia_centr,
        '"Наша Клиника", ООО': nasha_klinika,
        'КЛИНИКА СОВРЕМЕННОЙ МЕДИЦИНЫ г. Набережные Челны, ООО': klinika_sov_med_nab_chelni,
        ...
        '«Прайм-Стоматология», ООО': prime_stomatology,
        '«Дентерпрайз», ООО': denterprice,
        'ГБУЗ ЛО "Киришская КМБ"': gbuz_lo_kirishskaya_kmb,
        '"Он Клиник Рязань", ООО': on_clinic_ryazan,
        '«Клиника современной стоматологии «АРТ С», ООО': clinica_sov_stom_art_c,
        '«ЛДЦ Разумед», ООО': ldc_razumed,
        '«Сеть семейных медицинских центров Регион №2» (Рязань), ООО': ssmc_region_2_ryazan,
        '«Сеть семейных медицинских центров Регион №2» (Тула), ООО': ssmc_region_2_tula,
        '«Клиника «Уральская», ООО': clinic_uralskaya,
        'Центр профилактической медицины Ультрамед, ООО': centr_prof_med_ultramed,
        '«Мед-Арт» г. Томск, ООО': med_art_tomsk,
        'Стоматологическая клиника Карат, ООО': stomatology_clinic_karat,
        ...
        'Медицинский центр «Надежда», ООО': med_centr_nadezhda,
        '"Дент Арт" (Новокузнецк), ООО': dent_art_novokuzneck,
        'ГАУЗ СО «ГКБ №40»': gbauz_so_gkb_40,
        'АО «Камский диагностический центр «Медикам»': ao_kamskiy_dia_cent_medikam,
        '«Клиника ДНК», ООО': clinic_dnk,
        '«Мега-Центр», ООО': mega_cent,
        '«Охта Дентал плюс», ООО': ohta_dental_plus,
        '«Радуга-Мед», ООО': raduga_med,
        '«Сана Ко», ООО': sana_ko,
        '«Аня-Рыбинск», ООО': anay_ribinsk,
        'ДЦ «Клиника-Сити», ООО': dc_clinic_city,
        '"Мой зубной", ООО': moy_zubnoi,
        '«МЕДИН», ООО': medin,
        '«НЬЮ ДЕНТ» г. Тольятти, ООО': new_dent_toliati,
        'ЧУЗ Клиническая поликлиника РЖД-Медицина г. Архангельск': chuz_rzd_med_arhangelsk,
        '«Поликлиника №1 Вита Медикус», ООО': poly_1_vita_medikus,
        'Медицинский центр "СЭЛ", ООО': med_centr_sel,
        'Медицинский центр «Столица», ООО': med_cent_stolica,
        'МЛДЦ Диагност, ООО': mlcd_diagnost,
        'ГАУЗ «Городская клиническая больница №7» г. Казани': gauz_gkb_7_kazani,
        'МЦ «МЕДЕОР», ООО': mc_medeor,
        '«МедГид», ООО': medgit,
        '«НМЦ «Ваш Доктор», ООО': nmc_vash_doctor,
        '«Стоматолог» г. Калуга, ООО': stomatolog_kaluga,
        'Клиника Позвоночника на Авиаконструкторов, ООО': clinica_pozvonochnika_aviakonstruktorov,
        '«ФКВ клиника «Примавера», ООО': fkv_clinic_primavera,
        '«Клиника НоваВита», ООО': klinika_novavita,
        'Аквилио, ООО': akvilio,
        'Аквилио-НН,ООО': akvilio_nn,
        '«Нейро-ортопедический центр», ООО': nero_ortoped_centr,
        'Консультативно-диагностическая поликлиника, ООО': kons_dia_poly,
        'Офтальмологическая поликлиника, ООО': oftomolog_poly,
        '«МК «СОВА», ООО': mk_sova,
        '«ММК», ООО': mmk,
        '«ЛДК ВитаНова», ООО': ldc_vitanova,
        '«Стоматологический центр - Ниль и К «Стерлитамакский», ООО': stomat_center_nil_i_l_sterlitamakskiy,
        'ФГБУ НМИЦ эндокринологии Минздрава России': fgbu_nmic_endokrin_min_rf,
        'Славутич, ООО': slavutich,
        'СК «Болинет», ООО': sk_bolinet,
        'Доктор г. Красноярск, ООО': doctor_krasnoyarsk,
        '«32 карата», ООО': karata_32,
        '"Архком", ООО': arkhkom,
        'ГБУЗ МО «Серпуховская районная стоматологическая поликлиника»': gbuz_mo_serpuhovskaya_rsp,
        '"СЕМЕЙНЫЙ СТОМАТОЛОГ", ООО': semeyniy_stomat,
        'НИИ "Витадент", ООО': nii_vitadent,
        '«Клиника ЦМД», ООО': clinic_cmd,
        '«Медицинский лучевой центр», ООО': med_luchevoy_center,
        '«Консилиум», ООО': konsilium,
        '"ТАНМЕД", ООО': tanmed,
        '«РИАТ СПб», ООО': riat_spb,
        '«ЛДЦ МИБС – Тамбов», ООО': ldc_mibs_tambov,
        '"Стоматологическая клиника МВА", ООО': stomat_clinic_mba,
        '"Дентасервис", ООО': dentaservis,
        'ООО "Жемчужина" (Волгоград)': zemchuzina_volgograd,
    }
    if printer:
        summ = 0
...
def start_with_query(files_path={'path': [], 'file_name': [], 'folder_name': []}):  # noqa: CCR001, B006, E501
    ...
    list_of_dirs = DWH.select('select name,path from yandex_disk.clinics_files where status is null;')  # noqa
